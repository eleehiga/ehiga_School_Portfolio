...
const char altSymbols[] = {'v','w','x','y','z'};
...
int main() {
	...
	{
		randomlyPlaceShipsOnBoard(shipSymbols, shipSizes, p1ShipBoard);
	}
	randomlyPlaceShipsOnBoard(shipSymbols, shipSizes, p2ShipBoard);
	...
	while(winPlayerNumber == 0)
	{
		...
		if(p1Turn) // p1 affects p2's board
		{
			...
			updateBoard(targetRow, targetCol, p2ShipBoard, p2DisplayBoard, shipSymbols, altSymbols, shipSizes, isShipSunk); // Put a asterik or ship letters on board
		...
		else
		{
			...
			shipSunk = checkIfSunkShip(p1ShipBoard, shipSymbols, shipNames);
